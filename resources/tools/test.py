...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
    ...
    QTextEdit, QPushButton, QGroupBox, QFileDialog, QMessageBox, QDialog, QListWidget,QMenuBar,QToolBar,QInputDialog,QDialog,QInputDialog)
...
class DefectRegistrationApp(QMainWindow):
    def __init__(self):
        ...
        self.create_toolbar()
        # Cargar registros de excel
        self.load_excel_data()
        # Crear directorio de registros si no existe
        #directorio de desarrollo dentro de la misma app
        self.registros_dir = Path("registros")
        self.registros_dir.mkdir(exist_ok=True)

        # Ruta para el archivo CSV ( debug )
        #self.csv_file_path = self.registros_dir / "registros.csv"
         
        # Ru para el archivo CSV ( producción )
        self.csv_file_path = Path(r"H:\Publico\SMT-Prog-Status\Registos_retrabajo_SMT\registros.csv")
        
        if not self.csv_file_path.exists():
            self.create_csv_file()
            
        # Directorio de producción
        # self.registros_dir = Path(r"H:\Publico\SMT-Prog-Status\Registos_retrabajo_SMT") # por ejemplo (cambiar a disco H)
        # self.registros_dir.mkdir(exist_ok=True)        
        
        # Instanciamos SerialNumberManager
        self.serial_manager = SerialNumberManager()
        self.serial_manager.set_csv_path(self.csv_file_path)
        self.serials_by_job = {}  # Diccionario para almacenar números de serie por job
        ...
        # Contenedor horizontal para Información Básica y Ubicación
        basic_location_container = QHBoxLayout()
        ...
        self.serial_number.textChanged.connect(self.check_serial_length)  # Conectar señal
        basic_layout.addRow("S/N:", self.serial_number)

        # Atributo para almacenar la secuencia
        self.sequence = "N/A"  # Valor por defecto
        
        basic_group.setLayout(basic_layout)
        basic_location_container.addWidget(basic_group)
        
        # Sección 2: Ubicación y Clasificación
        location_group = QGroupBox("Ubicación y Clasificación")
        location_layout = QFormLayout()

        # Centro de Costo con mapeo automático de área
        ...
        self.area = QLabel("SMT")  # Valor inicial

        location_layout.addRow("Centro de Costo:", self.cost_center)
        location_layout.addRow("Área:", self.area)

        # Item y descripción con búsqueda automática
        self.item = QLineEdit()
        self.item.textChanged.connect(self.search_assembly_info)
        self.description = QLineEdit()
        self.description.setReadOnly(True)  # Hace el campo de solo lectura

        # Familia y Modelo (nuevos campos)
        self.family = QLineEdit()
        self.family.setReadOnly(True)  # Hace el campo de solo lectura
        self.model = QLineEdit()
        #self.model.setReadOnly(True)  # Hace el campo de solo lectura
        ...
        basic_location_container.addWidget(location_group)
        
        # Agregar el contenedor horizontal al layout principal
        main_layout.addLayout(basic_location_container)
        
        # Contenedor horizontal para Producto y Reparación
        product_repair_container = QHBoxLayout()

        # Sección 4: Información de Reparación (previously in this position)
        ...
        # Nuevo campo para mostrar el nombre del empleado
        self.employee_name = QLineEdit()
        self.employee_name.setReadOnly(True)  # Hace el campo de solo lectura

        self.repair_time = QSpinBox()
        self.repair_time.setSuffix("    minutos")
        self.repair_time.setRange(1, 999)
        ...
        repair_layout.addRow("Tiempo de reparación:", self.repair_time)
        repair_layout.addRow("Turno:", self.shift)

        repair_group.setLayout(repair_layout)
        product_repair_container.addWidget(repair_group)
        
        # Agregar el contenedor horizontal al layout principal
        main_layout.addLayout(product_repair_container)
        
        # Sección 5: Información del Defecto (con más espacio vertical)
        defect_group = QGroupBox("Información del Defecto")
        defect_layout = QFormLayout()
        
        self.fault_code = QComboBox()
        self.fault_code.addItems([
            "DESALINEADO INCLINADO",
            "FALTANTE NO COLOCADO",
            "UPSIDE-DOWN",
            "TIPO EQUIVOCADO",
            "PUENTE DE SOLDADURA",
            "DAÑADO MANEJO",
            ...
            "VALOR EQUIVOCADO",
            "REEMPLAZAR COMPONENTE POR CAMBIO EN ECO",
            "SOLDADURA FRIA",
            "SOLDADURA NO ADHIERE AL PAD",
            "COMPONENTE CON FISURA POR GOLPE",
            "POLARIDAD EQUIVOCADA"
        ])
        
        self.defect_description = QTextEdit()
        self.defect_description.setMinimumHeight(70)  # Aumentar altura mínima
        self.schematic_ref = QLineEdit()
        
        self.upload_button = QPushButton("Agregue foto del defecto (Si posee una foto del defecto)")
    ...
    def save_record(self):
        ...
        try:
            with open(self.csv_file_path, mode='a', newline='', encoding='latin1') as file:
                writer = csv.writer(file)
